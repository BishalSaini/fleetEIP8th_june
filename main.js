...
function choose_new_equ() {
    ...
    const selectFleetCategory1 = document.getElementById("selectFleetCategory1").value;
    ...
    const newfleet_luffing = document.getElementById("newfleet_luffing"); // Luffing input
    const newfleet_bedlength = document.getElementById("newfleet_bedlength"); // Bed Length input
    const prefetch = document.getElementById("prefetch");

    if (ac_dd.value === "New Equipment") {
        // Show relevant fields
        new_equip.style.display = newfleet_makemodel.style.display =
            newfleet_capinfo.style.display = "flex";

        new_equip.style.alignItems = newfleet_makemodel.style.alignItems =
            newfleet_capinfo.style.alignItems = "center";

        prefetch.style.display = "none"; // Hide prefetch section

        // Show/Hide inputs based on fleet category
        if (selectFleetCategory1 === "Aerial Work Platform") {
            newfleet_jib.style.display = "flex";
            newfleet_bedlength.style.display = "none"; // Hide bed length
        }
        else if (selectFleetCategory1 === "Trailor and Truck") {
            newfleet_jib.style.display = "none";
            newfleet_bedlength.style.display = "block"; // Show Bed Length
        } else {
            newfleet_jib.style.display = "flex";
            newfleet_bedlength.style.display = "flex"; // Show bed length for other categories
        }
    } else {
        // Hide all new equipment fields if not selected
        new_equip.style.display = newfleet_makemodel.style.display =
            newfleet_capinfo.style.display = newfleet_jib.style.display =
            newfleet_luffing.style.display = newfleet_bedlength.style.display = "none";

        // Show prefetch section
