...
$sql="SELECT * FROM `regional_office` where id='$id'";
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $sqledit="UPDATE `regional_office` SET `heading`='$heading',`address`='$address',`state`='$state' WHERE `id`='$editid'";
    $resultedit=mysqli_query($conn,$sqledit);

    $sqlalledit="UPDATE `rentalclients` SET `associated_regoffice`='$heading' where clientid='$clientidinput' and companyname='$companyname001' and address_type='Regional Office'";
    $resultalledit=mysqli_query($conn,$sqlalledit);
    if($resultedit && $resultalledit){
        session_start();
...
<form action="editregionaloffice.php" method="POST" class="editregionalofficeform" autocomplete="off">
    <div class="regionaleditformcontainer">
        <p class="headingpara">Edit Regional Office</p>
            ...
            <div class="trial1">
            <textarea name="address" id="" class="input02"><?php echo $row['address'] ?></textarea>
